...
async function notifyManagers(db, message) {
    const managers = await db('users').where({ role: 'manager' });
    for (const m of managers)
        await (0, db_1.createNotification)(m.id, message, 'approval');
}
async function notifyAdmins(db, message) {
    const admins = await db('users').where({ role: 'admin' });
    for (const a of admins)
        await (0, db_1.createNotification)(a.id, message, 'approval');
}
async function notifyProjectClients(db, projectId, message) {
    ...
    for (const c of clients)
        await (0, db_1.createNotification)(c.id, message, 'approval');
}
...
router.get('/', async (req, res) => {
    try {
        ...
        let query = db('approvals as ap')
            ...
            .select('ap.*', 't.title as task_title', 'p.name as project_name', 'c.name as client_name', 'sub.name as submitted_by_name', 'sub.avatar_color as submitted_by_color');
        if (role === 'manager') {
...
router.post('/', (0, auth_1.requireRoles)('admin', 'manager', 'employee'), async (req, res) => {
    ...
    if (!task_id || !title) {
        res.status(400).json({ error: 'task_id and title required' });
        return;
    }
    try {
        const db = (0, db_1.getDB)();
        ...
        if (existing) {
            res.status(409).json({ error: 'Approval already pending for this task' });
            return;
        }
    ...
    try {
        ...
        if (role === 'manager' && approval.status === 'pending_manager') {
            if (action === 'approve') {
                ...
                await (0, db_1.createNotification)(approval.submitted_by, `Your request "${approval.title}" was rejected by manager${notes ? ': ' + notes : ''}`, 'approval');
            }
            ...
            if (action === 'approve') {
                ...
                updates.rejection_notes = notes || null;
                await db('tasks').where({ id: approval.task_id }).update({ status: 'todo' });
                await (0, db_1.createNotification)(approval.submitted_by, `Your request "${approval.title}" was rejected by admin${notes ? ': ' + notes : ''}`, 'approval');
            }
            ...
            if (action === 'approve') {
                ...
                await (0, db_1.createNotification)(approval.submitted_by, `Your request "${approval.title}" was approved by the client. You can now start working on it.`, 'approval');
                ...
                updates.rejection_notes = notes || null;
                await db('tasks').where({ id: approval.task_id }).update({ status: 'todo' });
                await (0, db_1.createNotification)(approval.submitted_by, `Your request "${approval.title}" was rejected by client${notes ? ': ' + notes : ''}`, 'approval');
            }
            ...
            if (action === 'approve') {
                ...
                await (0, db_1.createNotification)(approval.submitted_by, `Your work on "${approval.title}" has been approved!`, 'approval');
            }
            else if (action === 'request_revision') {
                updates.status = 'revision_requested';
                updates.revision_notes = notes || null;
                await db('tasks').where({ id: approval.task_id }).update({ status: 'in_progress' });
                await (0, db_1.createNotification)(approval.submitted_by, `Revision requested on "${approval.title}"${notes ? ': ' + notes : ''}`, 'approval');
            }
        }
        else {
            res.status(403).json({ error: 'Not authorized for this action at current stage' });
            return;
        }
...
router.post('/:id/complete', (0, auth_1.requireRoles)('employee', 'manager', 'admin'), async (req, res) => {
    try {
        const db = (0, db_1.getDB)();
        const approval = await db('approvals').where({ id: req.params.id }).first();
        if (!approval) {
        ...
        if (approval.status !== 'work_in_progress') {
            res.status(400).json({ error: 'Task is not in work_in_progress stage' });
            return;
        }
