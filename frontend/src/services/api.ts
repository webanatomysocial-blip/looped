...
export const capacityApi = {
  ...
  declinedStages: () => api.get('/capacity/declined-stages'),
};
...
export const seoApi = {
  ...
  createShare: (clientId: number, data: { range: string; startDate?: string; endDate?: string; compareStart?: string; compareEnd?: string; demographics?: string[]; acquisitions?: string[]; country?: string; agency_name?: string }) => api.post(`/seo/share/${clientId}`, data),
  ...
  saveReport: (clientId: number, data: { name: string; range: string; start_date?: string; end_date?: string; compare_start?: string; compare_end?: string; country?: string; manual_snapshot?: any; agency_name?: string; acquisitions?: string[]; demographics?: string[] }) => api.post(`/seo/saved-reports/${clientId}`, data),
...
export const calendarApi = {
  ...
  updateSlotTime: (slotId: number, start_hour: number, slot_date?: string) => api.patch(`/calendar/slot/${slotId}/time`, { start_hour, ...(slot_date ? { slot_date } : {}) }),
...
export const xlr8Api = {
  ...
  stagePreDecline: (id: number, comment?: string) => api.post(`/xlr8/tickets/${id}/stage-pre-decline`, { comment }),
